...
SELECT * from INVOICE  -- Opening the entire Invoice Table when required.
...
Select * From Customer -- Opening the customer Table when required.
SELECT * from INVOICE  -- Opening the entire Invoice Table when required.
...
SELECT
	ARTIST.ARTIST_ID,
	ARTIST.NAME,
	COUNT(ARTIST.ARTIST_ID) AS COMPOSER
FROM
	TRACK
	JOIN GENRE ON TRACK.GENRE_ID = GENRE.GENRE_ID
